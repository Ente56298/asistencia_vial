...
const AssistanceButton: React.FC<AssistanceButtonProps> = ({ icon, label, onClick, isPremium, onMouseDown }) => (
    <button
        onClick={onClick}
        onMouseDown={onMouseDown}
        className="relative flex flex-col items-center justify-center p-4 bg-gray-800/70 border border-gray-700 rounded-xl hover:bg-gray-700/90 hover:border-cyan-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-cyan-400 ripple-effect"
    >
        {isPremium && (
...
const AssistancePanel: React.FC<AssistancePanelProps> = ({ onClose, onSelect, user }) => {
    const premiumAgents = [AssistanceType.Medical, AssistanceType.Legal];
            ...
            <div className="relative w-full max-w-2xl bg-gray-900/80 border border-cyan-500/30 rounded-2xl shadow-2xl flex flex-col">
                ...
                <div className="p-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <AssistanceButton
                            icon={<LegalIcon />}
                            label={AssistanceType.Legal}
                            onClick={() => onSelect(AssistanceType.Legal)}
                            isPremium={true}
                            onMouseDown={handleMouseDown}
                        />
                        <AssistanceButton
                            icon={<OtherIcon />}
                            label={AssistanceType.Other}
                            onClick={() => onSelect(AssistanceType.Other)}
                            isPremium={false}
                            onMouseDown={handleMouseDown}
                        />
                    </div>
